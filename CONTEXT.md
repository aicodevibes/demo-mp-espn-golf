...
- **Event State Context Provider**: Deep orchestration module that manages single-instance Firestore subscriptions, ESPN scoreboard and leaderboard fetching with adaptive polling (30–45s live, 5m completed), scoreboard caching, tournament normalization via the ESPN Tournament Adapter, Field Leaderboard Evaluator categorization, and Contest Engine evaluation in a single context provider — exposing one `useEventContext()` hook to all consuming routes. Owns the viewer event override for historical archive navigation.
- **Adaptive Leaderboard Poller**: Dynamic polling mechanism within the Event Context that switches between rapid refresh (30–45 seconds) during live tournament play and relaxed polling (5 minutes) for scheduled or completed tournaments.
